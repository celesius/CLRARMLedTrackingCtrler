...
{
	status = I2C_WriteBurst(MPU9250_ADDRESS, (uint8_t)address, (uint8_t *)data_write, (unsigned long)data_count);
...
bool CLRI2CInterface::read(int address, char *data_read, int data_count, char stop_flag)
{
	...
	status = I2C_ReadBurst(MPU9250_ADDRESS, (uint8_t)address, (uint8_t *)data_read, (unsigned long)data_count);
